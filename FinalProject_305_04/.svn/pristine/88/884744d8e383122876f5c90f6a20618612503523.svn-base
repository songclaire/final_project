...
<script type="text/javascript">
 $(function(){
 	$("#userNm").val("직원");
 	$("#userNmEn").val("Gyosuda");
 	$("#userRrn1").val("123456");
 	$("#userRrn2").val("1234567");
 	$("#userBir").val("2020-01-01");
 	...
 	$("#profId").val("20231234");
 	$("#userPass").val("1234");
 	$("#laboTel").val("021241235");
 	$("#entDate").val("2022-02-02");
 	$("#selPro").val("");
 	$("#entDate").val("");
 	$("#recordList0.recNm").val("입학");
 	$("#recordList0.recUpdate").val("2022-020-8");
 	$("#userBank").val("신한");
 	$("#userAcctHd").val("지은");
...
<form:form modelAttribute="empInfo" enctype="multipart/form-data" method="post">
				...
				<table class="tbl center">
					...
					<tbody>
						<tr>
							...
							<td><form:input path="empId" type="text" cssClass="form-control"  />
					        <form:errors path="empId" element="span" cssClass="text-danger" /></td>
						...
						<tr>
							<th scope="row" class="w100">건물<em class="red-txt asterisk">*</em></th>
						...
						<tr>
							...
							<c:choose>
								<c:when test="${not empty deptList }">
									...
									<option value="${deptName.deptId }" data-deptTel="${deptName.deptTel }">${deptName.deptNm }</option>
									</c:forEach>
								</c:when>
							</c:choose>
							</select>
							</td>
							<th scope="row">부서연락처<em class="red-txt asterisk">*</em></th>
							<td>
