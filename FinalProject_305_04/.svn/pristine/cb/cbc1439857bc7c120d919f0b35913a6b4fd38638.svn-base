...
<!-- 						<button type="button" class="btn btn-s purple" id="gradStdList">졸업예정자</button> -->
						</div>
					<div class="right-part">
						<div class="search-form">
							<form:form id="searchUI" modelAttribute="simpleCondition" onsubmit="return false;">
								...
								<div class="box-btn">
<!-- 									<button type="button" class="btn purple">메시지 발송</button> -->
								</div>
							</form:form>
						</div>
					</div>
...
let makeTrTag = function(student, index) {
	...
	return $('<tr>').append(
					$('<td>').append(checkBoxInput, checkBoxLabel)
						...
						,$('<td>').html(student.allAcquired == true ? '<span class="btn btn-s purple">충족완료</span>' : '<span class="btn btn-s default">미완료</span>' )
						,$('<td>').html(student.camYear == 4 ? '<span class="btn btn-s purple">졸업예정</span>' : '<span class="btn btn-s default">해당없음</span>')
	)
}
...
// 졸업예정자 보기
$('#gradStdList').on('click', function(){
	$.ajax({
		url : "${pageContext.request.contextPath}/emp/gradList",
		method : "get",
		data : {},
		dataType : "",
		success : function(resp) {
