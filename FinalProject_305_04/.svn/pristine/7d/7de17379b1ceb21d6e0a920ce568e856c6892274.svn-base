					...
					<tbody>
						<tr>
							<td>
								...
								<div class="rc-wrap">
									<div class="rc-inner">
										<input type="radio" id="meet1" name="consMeet" value="ONLINE" checked />
										<label for="meet1">대면</label>
									</div>
									<div class="rc-inner">
										<input type="radio" id="meet2" name="consMeet" value="OFFLINE" />
						...
						<tr>
							<td colspan="3">
								...
								<input class="form-control" name="consTitle" type="text" placeholder="제목을 입력해 주세요." autocomplete="off" />
							</td>
						</tr>
						<tr>
							<td colspan="3">
...
let makeConsultTag = function(consult, index){
	...
	let statusBtn = function(){
		...
		if(consult.consState==4){
			...
			btnTag = $("<button>").addClass("status wait-status").attr("data-id",consult.consId).attr("data-cons-id",consult.consId).html("대기")
		}
		return btnTag;
	}
