...
<div class="cont">
	...
	<div class="cont-title">
		<h2>교직원 정보관리<button type="button" class="star on"><span class="sr-only">즐겨찾기</span></button></h2>
		<!--cont-navi-->
		<div class="cont-navi">
		...
		<div class="cont-box-inner">
					...
					<div class="search-form">
						<form:form id="searchUI" modelAttribute="simpleCondition" onsubmit="return false;">
							<form:select path="searchType">
								...
								<form:option value="userId" label="사번" />
			...
			<div class="tbl-wrap">
				<table class="tbl center">
					...
					<tbody id="listBody">
						<tr>
							<c:set var="empInfoList" value="${pagingVO.dataList }" />
						</tr>
					</tbody>
				</table>
			</div>
...
let makeTrTag = function(empInfo, index) {
	let aTag = $("<a>")
				.attr("href", "${pageContext.request.contextPath}/emp/empInfoView?what=" + empInfo.empId)
				.html(empInfo.empId);
	...
	if (empInfo.levDate == null) {
		return $("<tr>").append(
				$("<td>").append(
					checkBoxInput, checkBoxLabel	
				)
				,$("<td>").html(empInfo.rnum)
				,$("<td>").html(aTag)
				,$("<td>").html(empInfo.userNm)
				,$("<td>").html(empInfo.deptVO.deptNm)
				,$("<td>").html(empInfo.pos)
				,$("<td>").html("재직")
		)
...
let searchForm = $('#searchForm').on('submit', function(event){
	...
	$.ajax({
		...
		success : function(resp) {
			...
			if(dataList){
				$.each(dataList, function(index,empInfo){
					trTags.push(makeTrTag(empInfo,index));
				});
			} else {
				let tr = $("<tr>").html(
...
$(".insertBtn").on('click', function(event){
	event.preventDefault();
	let url = "${pageContext.request.contextPath}/emp/empInfoInsert";
	location.href = url;
});
