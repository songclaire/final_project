	...
	<div class="white-box">
		...
		<div class="tob-box">
			...
			<div class="tbl-wrap">
				...
				<div class="title">
					<div class="box-btn">
						<a href="${pageContext.request.contextPath }/student/assignment?curlectId=${currLectVO.curlectId }" class="btn default">목록</a>
						<!-- 현재년도 -->
						<c:set var="now" value="<%=new java.util.Date()%>" />
						<fmt:formatDate var="sysYear" value="${now}" pattern="yyyy-MM-dd" />
						<c:choose>
							<c:when test="${sysYear lt assignmentVO.assignDdate}"> <!-- 제출기간 만료전  -->
								<form id="delForm" action="${pageContext.request.contextPath }/student/assignment/assignmentView?asgnId=${assignmentVO.asgnId }&curlectId=${assignmentVO.assignmentSubmitVO.curlectId }" method="post">
									<input type="hidden" name="_method" value="delete" />
									<button type="button" class="btn red"  onclick="f_movedelete()">삭제</button>
								</form>
								<a href="${pageContext.request.contextPath}/student/assignment/assignmentEdit?asgnId=${param.asgnId}&curlectId=${param.curlectId}" class="btn purple">수정</a>			
							</c:when>
						</c:choose>
					</div>
				</div>
