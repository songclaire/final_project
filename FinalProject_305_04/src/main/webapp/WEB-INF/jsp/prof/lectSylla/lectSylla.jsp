...
// 					console.log("lectRoomId : " + lectRoomVO.lectRoomId + ", roomNum : " + lectRoomVO.roomNum);
	...
	$("#syllaForm").on("submit", function(event) {
		...
		/*
		let data = {
			"semeId":$('#selSemeYear').val(),
			"subMajorVO":{
				"subMajorId":$('#selSubMajor').val()
			},
			"commDesc":$('input[name=commDesc]').val(),
			"camYear":$('#selCamYear').val(),
			"lectHour":$('input[name=lectHour]').val(),
			"lectNm":$('#lectNm').val(),
			"lectCom":$('#lectCom').val(),
			...
			"note":$('#note').val(),
			"buildingVO":{
				"buildId":$('#selBuild').val()
			},
			"lectRoomVO":{
				"lectRoomId":$('#selLectRoom').val()
			},
			"hopeTime":$('input[name=hopeTime]').val(),
			"professorVO":{
				"profId":$('#profId').val(),
			},
			"scoreCritList":{
				"scoreCritNm":$('input[name=scoreCritList[0].scoreCritNm]').attr('id'),
				"scoreCritRatio":$('input[name=scoreCritList[0].scoreCritRatio]').val()
			}
		}
		*/
...
<div class="cont">
	...
	<div class="white-box">
						...
						<td>
							<div class="display-tbl">
								<div class="display-tblCell">
									...
									<input type="text" name="lectWeekList[0].lectWeekly" class="w100">
								</div>
							</div>
						</td>
					</tr>
		...
		<input id='profId' name="professorVO.profId" value="3120001" />
	</form>
	</div>
</div>