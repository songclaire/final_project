						...
						<tr>
							<c:choose>
								...
								<c:when test="${usrVO.userRole eq 'ROLE_PRF'}">
									<th scope="row">소속</th>
									<td>${infoVO.majorList.collList.collNm } ${infoVO.majorList.majorNm }</td>
									<th scope="row">상태</th>
							...
							<c:when test="${not empty userCertList }">
								<c:forEach items="${userCertList }" var="cert">
									<tr>
										...
										<td>
											<button name="btnIssue" data-user-id="${usrVO.userId }" data-cert-charge="${cert.certCharge }" data-cert-nm="${cert.certNm }" data-cert-id="${cert.certId }" 
...
// 				};
				...
				$.ajax({
					...
					success : function(resp) {
						...
						location.href = "https://localhost/campus/certificateBox";
					},
